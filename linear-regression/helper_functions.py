...
def linear_regression_pipeline(
    ...
    if target_transformer:
        y_train = target_transformer.inverse_transform(y_train.reshape(-1,1))
...
def backward_stepwise_train(X_train, y_train, feature_cols):
    ...
    rmse = np.sqrt(mean_squared_error(target_transformer.inverse_transform(yval.reshape(-1,1)), target_transformer.inverse_transform(model.predict(xval[feature_cols]).reshape(-1,1))))
...
def build_lasso_model(X_train, y_train, alpha):
    ...
    yval = target_transformer.inverse_transform(yval.reshape(-1,1))
    preds = target_transformer.inverse_transform(preds.reshape(-1,1))
